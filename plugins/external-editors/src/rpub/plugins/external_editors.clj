...
(ns rpub.plugins.external-editors
    ...
    (merge
      ...
      (case method-name
        ...
        (let [[_blog-id username password constraints] method-params]
          {:username username
           :password password
           :constraints (update-keys constraints csk/->kebab-case-keyword)})
        ...
        (let [[_blog-id username password post-fields] method-params]
          {:username username
           :password password
           :post-fields (update-keys post-fields csk/->kebab-case-keyword)})
...
(defn get-post-response
  [{:keys [::content-types/model] :as req}
...
(defn new-post-response [{:keys [::content-types/model identity]} method-call]
  (let [{:keys [post-fields]} method-call
        post-fields' (update post-fields :post-type
                             (fn [post-type]
                               (-> (or post-type "post")
                                   inflections/plural
                                   keyword)))
        [content-type] (content-types/get-content-types
                         model
    ...
    (content-types/create-content-item! model new-content-item)
    [:methodResponse
     [:params
      [:param
       [:value
...
(defn edit-post-response [{:keys [::content-types/model identity]} method-call]
  (let [{:keys [post-id post-fields]} method-call
        ...
        updated-content-item (-> existing-content-item
                                 ...
                                 (merge {:updated-at (Instant/now)
                                         :updated-by (:id identity)})
                                 (select-keys [:id :document :created-at
                                               :created-by :updated-at
                                               :updated-by]))]
    (content-types/update-content-item! model updated-content-item)
    [:methodResponse
     [:params
      [:param
       [:value
...
(defn delete-post-response [{:keys [::content-types/model]} method-call]
  (let [{:keys [post-id]} method-call
        [existing-content-item] (content-types/get-content-items
                                  model
                                  {:content-item-ids [post-id]})]
    (content-types/delete-content-item! model existing-content-item)
    [:methodResponse
     [:params
      [:param
       [:value
...
(defn rsd-response [{:keys [site-base-url]}]
  (let [api-link (str site-base-url "/xmlrpc.php")]
    (xml-response
...
(defn wrap-rsd-link-tag [handler]
  (fn [{:keys [site-base-url] :as req}]
    (let [link-tag [:link {:rel "EditURI"
                           ...
                           :title "RSD"
                           :href (str site-base-url "/xmlrpc.php?rsd")}]
...
(defn xmlrpc-post [{:keys [model body] :as req}]
  (let [method-call (->method-call (parse-xml body))]
    (if-let [identity (authorize method-call model)]
      (let [req' (assoc req :identity identity)]
        (method-response req' method-call))
      (forbidden-response))))

(defn routes [opts]
  (let [opts' (assoc opts :auth-required false)]
    [["/xmlrpc.php" {:get xmlrpc-get
                     :post xmlrpc-post
                     :middleware (admin/admin-middleware opts')}]]))

(defn plugin [_]
  {:name "External Editors"
   :description (str "Add support to view and edit posts from external editors "
                     "(e.g. MarsEdit).")
   :middleware [wrap-rsd-link-tag]
   :routes routes})
